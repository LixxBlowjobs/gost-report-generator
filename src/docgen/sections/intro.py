...
def generate(analysis: dict, metadata: dict, plan: dict = None) -> dict:
    user = SECTION_PROMPTS["intro"].format(
        analysis=json.dumps(analysis, indent=2, ensure_ascii=False),
        metadata=json.dumps(metadata, indent=2, ensure_ascii=False),
        intro_thesis=json.dumps((plan or {}).get("intro_thesis", []),
                                ensure_ascii=False),
    )
    return generate_json(SECTION_PROMPTS["intro"], user,
                         max_tokens=3072, label="Введение")


def build(doc, result: dict, styles):
    styles.make_paragraph(doc, "ВВЕДЕНИЕ", font_size=14, bold=True,
